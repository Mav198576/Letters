...
with tab2:
    st.header("📇 Leads")
    search_leads = st.text_input("🔍 Search Leads", placeholder="Name or email", key="search_leads_phase7a")
...
with tab3:
    st.header("👥 Clients")
    search_clients = st.text_input("🔍 Search Clients", placeholder="Name or email", key="search_clients_phase7b")
...
with tab1:
    ...
    # Trend with emojis
    trend = np.random.choice(["📉", "📈"], size=len(score_data))
    score_df["Trend"] = trend
    st.dataframe(score_df)

    st.line_chart(score_df[["Credit Score"]])

    st.subheader("🗓️ Calendar (Basic)")
    st.date_input("Select a date", value=datetime.today())

    st.subheader("⚠️ Clients Needing Report Refresh (43+ Days)")
    st.markdown("Clients who are approaching the 45-day mark since last credit report upload.")
    ...
    if nearing_refresh:
        for r in nearing_refresh:
            st.warning(f"**{r['name']}** ({r['email']}) — {r['days']} days since last report")
    else:
