...
def get_user():
    return {"email": "guest@example.com", "is_admin": True}

user = get_user()
consumer_key = "guest"

# Prevent NameError if selected_accounts is not yet defined
selected_accounts = []
...
# ========== PORTAL VIEW ==========
if get_user().get("is_admin", False):
...
if not get_user().get("is_admin", False):
    ...
    agency_brand = st.sidebar.text_input("Your Agency Name", value=branding_data.get(get_user().get("email", "guest@example.com"), {}).get("name", ""))
    agency_logo = st.sidebar.text_input("Logo URL", value=branding_data.get(get_user().get("email", "guest@example.com"), {}).get("logo", ""))
    if st.sidebar.button("Save Branding"):
        branding_data[get_user().get("email", "guest@example.com")] = {"name": agency_brand, "logo": agency_logo}
...
if not get_user().get("is_admin", False):
    consumer_key = get_user().get("email", "guest@example.com").replace("@", "_").replace(".", "_")
...
client_brand_name = branding_data.get(get_user().get("email", "guest@example.com"), {}).get("name", "")
client_logo = branding_data.get(get_user().get("email", "guest@example.com"), {}).get("logo", "")
...
if get_user().get("is_admin", False) and st.sidebar.button("📧 Send All Due Reminders"):
...
if uploaded_file:
    trigger_zapier("report_uploaded", {"user": get_user().get("email", "guest@example.com"), "score": credit_score})

# Trigger when PDF letter is downloaded
if selected_accounts and st.button("Trigger Zapier for Letter"):
    trigger_zapier("letter_generated", {"user": get_user().get("email", "guest@example.com"), "creditors": [x["creditor"] for x in selected_accounts]})
...
if not get_user().get("is_admin", False):
    ...
    if st.button("Send Message"):
        all_messages.append({
            "from": get_user().get("email", "guest@example.com"),
            "to": "admin",
            "text": msg,
...
logo_path = None
if uploaded_logo:
    logo_path = f"logos/{get_user().get("email", "guest@example.com").replace('@', '_at_')}_logo.png"
...
# Report viewer
    ...
    client_key = get_user().get("email", "guest@example.com").replace("@", "_at_")
...
# Settings view
    ...
    st.markdown(f"**Email:** {get_user().get("email", "guest@example.com")}")
    st.markdown("**Branding Options**")
    agency_name = st.text_input("Agency Name", value=branding_data.get(get_user().get("email", "guest@example.com"), {}).get("name", ""))
    color_theme = st.color_picker("Theme Color", value="#2c3e50")
    new_logo = st.file_uploader("Upload New Logo", type=["png", "jpg"])

    if st.button("Save Settings"):
        branding_data[get_user().get("email", "guest@example.com")] = {
            "name": agency_name,
            "color": color_theme,
            "logo": branding_data.get(get_user().get("email", "guest@example.com"), {}).get("logo", "")
        }
        if new_logo:
            logo_path = f"logos/{get_user().get("email", "guest@example.com").replace('@', '_at_')}_logo.png"
            with open(logo_path, "wb") as f:
                f.write(new_logo.read())
            branding_data[get_user().get("email", "guest@example.com")]["logo"] = logo_path
...
if selected_tab == "Leads" and get_user().get("is_admin", False):
